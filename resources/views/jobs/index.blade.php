<x-layout>
    ...
    <form method="GET" action="/jobs" class="bg-white rounded-xl shadow-sm border border-gray-200 p-5 mb-6">
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
            <input
                ...
                placeholder="Search by title or salary..."
            ...
            <div class="flex gap-2">
                ...
                <a href="/jobs" class="flex-1 bg-gray-100 hover:bg-gray-200 text-gray-700 font-semibold py-2.5 px-4 rounded-lg text-sm transition text-center">Reset</a>
            </div>
        </div>
    </form>
    ...
    @if($jobs->isEmpty())
        <div class="text-center py-16 bg-white rounded-xl border border-gray-200">
            ...
            <p class="mt-4 text-gray-500 text-lg">No jobs found matching your criteria.</p>
            <a href="/jobs" class="mt-4 inline-block text-blue-600 hover:underline">Clear filters</a>
        </div>
    @else
        <div class="space-y-4">
            @foreach($jobs as $job)
                <a href="/jobs/{{ $job->id }}" class="block bg-white rounded-xl border border-gray-200 p-5 hover:border-blue-300 hover:shadow-md transition group">
                    <div class="flex items-start justify-between">
                        <div class="flex-1">
                            <div class="flex items-center gap-2 mb-1">
                                <span class="text-xs font-semibold text-blue-600 bg-blue-50 px-2 py-0.5 rounded-full">{{ $job->employer->name }}</span>
                                @if($job->industry)
                        ...
                        <span class="text-xs text-gray-400 ml-4 whitespace-nowrap">{{ $job->created_at->diffForHumans() }}</span>
                    </div>
                </a>
            @endforeach
        </div>
