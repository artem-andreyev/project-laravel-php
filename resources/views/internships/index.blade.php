<x-layout>
    <x-slot:heading>{{ __('internships.listingsHeading') }}</x-slot:heading>
    ...
    <form method="GET" action="/internships" class="bg-white rounded-xl shadow-sm border border-gray-200 p-5 mb-6">
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
            <input
                ...
                placeholder="{{ __('internships.searchPlaceholder') }}"
            ...
            <select name="duration" class="border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:ring-2 focus:ring-blue-500 outline-none">
                <option value="">{{ __('internships.anyDuration') }}</option>
                <option value="1" {{ request('duration') == '1' ? 'selected' : '' }}>1 {{ __('internships.month') }}</option>
                <option value="2" {{ request('duration') == '2' ? 'selected' : '' }}>2 {{ __('internships.months') }}</option>
                <option value="3" {{ request('duration') == '3' ? 'selected' : '' }}>3 {{ __('internships.months') }}</option>
                <option value="6" {{ request('duration') == '6' ? 'selected' : '' }}>6 {{ __('internships.months') }}</option>
                <option value="12" {{ request('duration') == '12' ? 'selected' : '' }}>12 {{ __('internships.months') }}</option>
            </select>
            <input
                type="text"
                name="location"
                value="{{ request('location') }}"
                placeholder="{{ __('internships.locationPlaceholder') }}"
                class="border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:ring-2 focus:ring-blue-500 outline-none"
            >
            <div class="flex gap-2">
                <button type="submit" class="flex-1 bg-blue-600 hover:bg-blue-700 text-white font-semibold py-2.5 px-4 rounded-lg text-sm transition">{{ __('buttons.search') }}</button>
                <a href="/internships" class="flex-1 bg-gray-100 hover:bg-gray-200 text-gray-700 font-semibold py-2.5 px-4 rounded-lg text-sm transition text-center">{{ __('buttons.reset') }}</a>
            </div>
        </div>
    </form>
    ...
    @if($internships->isEmpty())
        <div class="text-center py-16 bg-white rounded-xl border border-gray-200">
            ...
            <p class="mt-4 text-gray-500 text-lg">{{ __('internships.noMatching') }}</p>
            <a href="/internships" class="mt-4 inline-block text-blue-600 hover:underline">{{ __('internships.clearFilters') }}</a>
        </div>
    @else
        <div class="space-y-4">
                ...
                <div class="relative bg-white rounded-xl border border-gray-200 hover:border-blue-300 hover:shadow-md transition group">
                        ...
                        <div class="flex items-start justify-between">
                            <div class="flex-1 pr-10">
                                <div class="flex items-center gap-2 mb-1">
                                    <span class="text-xs font-semibold text-blue-600 bg-blue-50 px-2 py-0.5 rounded-full">{{ $internship->employer->name ?? __('companies.generic') }}</span>
                                </div>
                    ...
                    @auth
                        ...
                        <form method="POST" action="/saved/toggle" class="absolute top-4 right-4">
                            ...
                            <button type="submit" title="{{ $saved ? __('saved.remove') : __('saved.saveInternship') }}"
