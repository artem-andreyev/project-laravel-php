<x-layout>
    <x-slot:heading>{{ __('auth.login') }}</x-slot:heading>
    ...
    <div class="max-w-md mx-auto">
        <div class="bg-white rounded-2xl border border-blue-100 shadow-sm overflow-hidden">
            ...
            <div class="px-8 py-6 border-b border-gray-100 bg-gradient-to-r from-blue-50 to-indigo-50 text-center">
                ...
                <h2 class="text-lg font-bold text-gray-900">{{ __('auth.welcomeBack') }}</h2>
                <p class="text-sm text-gray-500 mt-1">{{ __('auth.loginSubtitle') }}</p>
            </div>
            ...
            <form method="POST" action="/login" class="px-8 py-6 space-y-5">
                ...
                <div>
                    <label for="email" class="block text-sm font-semibold text-gray-700 mb-1.5">{{ __('auth.email') }}</label>
                    <div class="relative">
                ...
                <div>
                    <label for="password" class="block text-sm font-semibold text-gray-700 mb-1.5">{{ __('auth.password') }}</label>
                    <div class="relative">
                        <span class="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400">
                ...
                <p class="text-center text-sm text-gray-500">
                    {{ __('auth.noAccount') }}
                    <a href="/register" class="text-blue-600 font-semibold hover:underline">{{ __('auth.register') }}</a>
                </p>
            </form>
        </div>
    </div>
